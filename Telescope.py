...
class Nickel(Telescope):
    ...
    def compute_sn_exposure(self, sn):
        ...
        if i_exp <= 600:
            exposures.update({Constants.i_prime: self.round_to_num(Constants.round_to, i_exp)})
        if V_exp <= 600:
            exposures.update({Constants.V_band: self.round_to_num(Constants.round_to, V_exp)})
        if B_exp <= 600:
            exposures.update({Constants.B_band: self.round_to_num(Constants.round_to, B_exp)})
